-- ============================================================================
-- PHASE 5 PREREQUISITE: CREATE CARE_TARGETS TABLE
-- ============================================================================
-- This table stores individual care targets (complaints) within episodes,
-- enabling complaint-level outcome analysis as required by Phase 5.
-- ============================================================================

-- Create care_targets table
CREATE TABLE public.care_targets (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  episode_id TEXT NOT NULL,
  name TEXT NOT NULL,
  domain TEXT,  -- MSK or Neuro
  body_region TEXT,
  status TEXT NOT NULL DEFAULT 'ACTIVE',  -- ACTIVE, MONITOR, MAINTENANCE, DISCHARGED
  priority INTEGER DEFAULT 1,
  
  -- Outcome association
  outcome_instrument TEXT,  -- NDI, ODI, QuickDASH, LEFS, etc.
  ...
  -- Constraints
  CONSTRAINT valid_status CHECK (status IN ('ACTIVE', 'MONITOR', 'MAINTENANCE', 'DISCHARGED'))
);
...
-- RLS Policies
CREATE POLICY "Clinicians can view care targets"
ON public.care_targets FOR SELECT
USING (has_role(auth.uid(), 'clinician') OR has_role(auth.uid(), 'admin'));

CREATE POLICY "Clinicians can insert care targets"
ON public.care_targets FOR INSERT
WITH CHECK (has_role(auth.uid(), 'clinician') OR has_role(auth.uid(), 'admin'));

CREATE POLICY "Clinicians can update care targets"
ON public.care_targets FOR UPDATE
USING (has_role(auth.uid(), 'clinician') OR has_role(auth.uid(), 'admin'));

CREATE POLICY "Clinicians can delete care targets"
ON public.care_targets FOR DELETE
USING (has_role(auth.uid(), 'clinician') OR has_role(auth.uid(), 'admin'));

-- Indexes
CREATE INDEX idx_care_targets_episode_id ON public.care_targets(episode_id);
CREATE INDEX idx_care_targets_status ON public.care_targets(status);

-- Updated_at trigger
CREATE TRIGGER update_care_targets_updated_at
BEFORE UPDATE ON public.care_targets
FOR EACH ROW
...
-- View 1: Care Target Outcomes Table (Primary Analytic Unit)
CREATE OR REPLACE VIEW public.analytics_care_target_outcomes AS
SELECT 
...
-- View 2: Episode Summary View
CREATE OR REPLACE VIEW public.analytics_episode_summary AS
SELECT 
  e.id AS episode_id,
  e.patient_name,
  e.clinic_id,
...
-- View 3: Registry Export View (Research-Ready)
CREATE OR REPLACE VIEW public.analytics_registry_export AS
SELECT 
  ct.id AS care_target_id,
  ct.episode_id,
  ct.name AS care_target_name,
