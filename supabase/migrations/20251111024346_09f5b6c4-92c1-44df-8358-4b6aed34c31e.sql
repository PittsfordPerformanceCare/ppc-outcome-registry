...
DROP POLICY IF EXISTS "Admins can manage thresholds" ON public.pending_episode_thresholds;
DROP POLICY IF EXISTS "Users can view thresholds for their clinic" ON public.pending_episode_thresholds;

-- Drop trigger if exists
DROP TRIGGER IF EXISTS update_pending_episode_thresholds_updated_at ON public.pending_episode_thresholds;

-- Drop table if exists
DROP TABLE IF EXISTS public.pending_episode_thresholds;

-- Create pending_episode_thresholds table for configurable settings
CREATE TABLE public.pending_episode_thresholds (
